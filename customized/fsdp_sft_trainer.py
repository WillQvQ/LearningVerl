...
class FSDPSFTTrainer:
    ...
    def fit(self):
        ...
        # 创建全局进度条，基于总步数
        pbar = tqdm(total=self.total_training_steps, disable=rank != 0, desc="Training Progress")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for data in self.train_dataloader:
                ...
                # 更新全局进度条
                if rank == 0:
                    pbar.set_description(f"Epoch {epoch + 1}/{self.config.trainer.total_epochs}, Step {global_step}/{self.total_training_steps}")
                    pbar.update(1)
                ...
                if rank == 0:
                    ...
                    # 可选：在进度条中显示当前损失
                    pbar.set_postfix({
                        'loss': f"{metric['train/loss']:.4f}",
                        'lr': f"{metric['train/lr(1e-3)']:.3f}e-3"
                    })
                ...
                if is_last_step or (self.config.trainer.test_freq > 0 and is_valid_step):
                    ...
                    if rank == 0:
                        ...
                        # 在进度条中也显示验证损失
                        pbar.set_postfix({
                            'train_loss': f"{metric.get('train/loss', 0):.4f}",
                            'val_loss': f"{metric['val/loss']:.4f}",
                            'lr': f"{metric.get('train/lr(1e-3)', 0):.3f}e-3"
                        })
                    torch.distributed.barrier()
...
def create_sft_dataset(data_paths, data_config, tokenizer):
    """Create a dataset."""
    # build dataset
    # First check if multi-region configuration is enabled
    if "multi_region_config" in data_config:
        from verl.utils.dataset.multi_region_sft_dataset import MultiRegionLossMaskDataset
        dataset_cls = MultiRegionLossMaskDataset
    # Then check if a custom dataset class is specified
    elif data_config.custom_cls.get("path", None):
