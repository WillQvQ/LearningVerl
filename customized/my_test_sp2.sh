set -x

if [ "$#" -lt 2 ]; then
    echo "Usage: run_qwen_05_sp2.sh <nproc_per_node> <save_path> [other_configs...]"
    exit 1
fi

nproc_per_node=$1
save_path=$2

# Shift the arguments so $@ refers to the rest
shift 2
export PYTHONPATH="/remote-home1/moss/LearningVerl/verl"
torchrun --standalone --nnodes=1 --nproc_per_node=$nproc_per_node \
    /remote-home1/moss/LearningVerl/verl/verl/trainer/fsdp_sft_trainer.py \
    data.train_files=/remote-home1/share/data/gsm8k_verl/train.parquet \
    data.val_files=/remote-home1/share/data/gsm8k_verl/test.parquet \
    data.prompt_key=extra_info \
    data.response_key=extra_info \
    optim.lr=1e-4 \
    data.prompt_dict_keys=['question'] \
    data.response_dict_keys=['answer'] \
    data.micro_batch_size=4 \
    model.partial_pretrain=/remote-home1/share/models/Qwen2.5-0.5B-Instruct \
    trainer.default_local_dir=$save_path \
    trainer.project_name=gsm8k-sft \
    trainer.experiment_name=gsm8k-sft-qwen-2.5-0.5b-instruct-sp2 \
    trainer.logger=['console'] \
    trainer.total_training_steps=1000 \
    trainer.default_hdfs_dir=null $@ \
    ulysses_sequence_parallel_size=2 \
    use_remove_padding=true

    # -m verl.trainer.fsdp_sft_trainer.py \